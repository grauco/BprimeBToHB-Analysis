...
void NormComparison_Data(){

  //TFile *file = new TFile("output/singleh/histos/QCDfromData_v2_cat_inclusive_singleH.root", "RECREATE");

  TFile *qcd = new TFile("output/singleh/histos/QCD_v2_cat_inclusive_singleH.root");
  TFile *tt = new TFile("output/singleh/histos/TT_v2_cat_inclusive_singleH.root");
  TFile *data = new TFile("output/singleh/histos/Data_cat_inclusive_singleH.root");
  ...
  TH1F *htt_ht = (TH1F*)tt->Get("h_Ht");
  TFile *file = new TFile("output/singleh/histos/QCDfromData_v2_cat_inclusive_singleH.root", "UPDATE");
  ...
  float sf = h_bprimemass_b->Integral()/h_bprimemass_d->Integral();
  h_bprimemass_c->Scale(sf);
  h_bprimemass_a->Add(htt_ht,+1);
    
  //h_bprimemass_c->Write();
  //file->Write();
  //file->Close();
  //data->GetList()->Write();
  //h_bprimemass_c->SetDirectory(0);
  TH1F *h_Ht=(TH1F*)h_bprimemass_c->Clone();
  ...
  //h_bprimemass_a->SetMinimum(0);
  //  h_bprimemass_c->SetMinimum();
  ...
  /*  leg = new TLegend(0.35,0.65,0.85,0.85); 
  leg->SetTextSize(0.03);
  leg->SetFillColor(0);
  leg->SetBorderSize(0);
  leg->AddEntry(h_bprimemass_c,"QCD estimation from data","f");
  leg->AddEntry(h_bprimemass_a,"QCD true from MC","l");
  leg->Draw();*/
  ...
  leg->SetFillColor(0);
  leg->SetBorderSize(0);
  leg->AddEntry(h_bprimemass_c,"expected QCD from Data","l");
  ...
  //TH1F *h_bprimemass_c_data = (TH1F*)h_bprimemass_c->Clone();
  //cout << h_bprimemass_c_data->Integral() << endl;
 ...
 c1->Print("Plots_bkg/BkgEst_norm_AC_Data_withTT_cat_inclusive.pdf");
    
 h_bprimemass_c->SetName("h_Ht");
 h_bprimemass_c->Write("h_Ht",TObject::kWriteDelete);
 // h_Ht->Write();
 file->Write();
 file->Close();
